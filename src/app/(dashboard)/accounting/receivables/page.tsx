...
export default async function ReceivablesPage() {
  ...
  const totalCollected           = collectedFromOutstanding;
    ...
    <main className="min-h-full bg-slate-50">
      <div className="mx-auto max-w-[1600px] space-y-6 px-4 py-4 sm:px-6 sm:py-6 lg:px-4 lg:py-6">
        ...
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <StatCard title="Collected So Far"     value={formatAED(totalCollected)}    sub="Partial payments on open invoices"   icon={<Wallet size={20} />}       tone="emerald" />
        ...
        <Card className="overflow-hidden">
          <div className="border-b border-slate-200 px-6 py-4">
            ...
            <p className="mt-1 text-sm text-slate-500">{creditNotes.length} credit notes issued against invoices</p>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  {["CN Number", "Against Invoice", "Customer", "Date", "Amount", "VAT", "Total", "Reason"].map((h) => (
                    <th key={h} className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {creditNotes.length === 0 && (
                  <tr><td colSpan={8} className="px-6 py-10 text-center text-sm text-slate-400">No credit notes issued yet</td></tr>
                )}
                {creditNotes.map((cn) => (
                  <tr key={cn.id} className="border-b border-slate-100 transition hover:bg-slate-50/80">
                    ...
                    <td className="px-5 py-4">
                      {cn.invoice
                        ? <span className="font-mono text-sm font-semibold text-sky-700">{cn.invoice.number}</span>
                        : <span className="text-slate-300 text-sm">—</span>}
                    </td>
                    <td className="px-5 py-4 text-sm font-medium text-slate-800">{cn.customer?.name ?? "—"}</td>
